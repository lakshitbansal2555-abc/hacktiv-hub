  ...
  return (
    <main className="min-h-screen bg-terminal text-terminal-foreground terminal-grid terminal-scanlines">
      ...
      <section id="work" className="mx-auto max-w-7xl px-5 py-24 lg:px-8">
        <div data-reveal><SectionHeading eyebrow="03 / selected work" title="Systems with a purpose." /><div className="mt-12 border border-neon/40 bg-terminal-panel panel-glow"><div className="flex flex-wrap items-center justify-between gap-4 border-b border-border px-5 py-4 font-display text-[11px] uppercase tracking-[0.12em]"><span className="flex items-center gap-2 text-neon"><Braces size={14} /> open_source / featured</span><span className="text-terminal-muted">PR merged</span></div><div className="grid gap-10 p-6 sm:p-8 lg:grid-cols-[1fr_0.8fr] lg:items-center"><div><h3 className="font-display text-2xl text-terminal-foreground">IntelliView Orchestrator</h3><p className="mt-2 font-display text-sm text-cyan">FastAPI Backend · SQLAlchemy</p><p className="mt-5 max-w-xl leading-7 text-terminal-muted">Refactored API endpoints around a reusable <span className="font-display text-terminal-foreground">get_db()</span> dependency-injection pattern, replacing manual session handling and adding unit tests for the service-layer change.</p><a href="https://github.com/rajat-wyrm/intelliview-orchestrator" target="_blank" rel="noreferrer" className="mt-7 inline-flex items-center gap-2 font-display text-xs uppercase tracking-[0.12em] text-neon hover:underline">View contribution <ExternalLink size={14} /></a></div><div className="grid grid-cols-3 gap-3 font-display text-center text-[10px] uppercase tracking-[0.1em] text-terminal-muted"><div className="border border-border p-4"><Code2 size={17} className="mx-auto mb-3 text-neon" /><span>refactor</span></div><div className="border border-border p-4"><Check size={17} className="mx-auto mb-3 text-cyan" /><span>tests</span></div><div className="border border-border p-4"><GitHub size={17} className="mx-auto mb-3 text-amber" /><span>merged</span></div></div></div></div></div>
        <div data-reveal className="mt-20"><div className="flex items-end justify-between gap-6"><SectionHeading eyebrow="04 / project archive" title="Projects in the field." /><a href={githubUrl} target="_blank" rel="noreferrer" className="hidden items-center gap-2 font-display text-xs uppercase tracking-[0.1em] text-terminal-muted transition-colors hover:text-neon sm:flex">all repositories <ArrowUpRight size={14} /></a></div><div className="mt-10 grid gap-px border border-border bg-border sm:grid-cols-2 lg:grid-cols-4">{projects.map((project, index) => <a key={project.title} href={githubUrl} target="_blank" rel="noreferrer" className="group flex min-h-64 flex-col bg-terminal p-6 transition-colors hover:bg-terminal-panel"><div className="flex items-center justify-between font-display text-[10px] text-terminal-muted"><span>0{index + 1}</span><ArrowUpRight size={15} className="text-border transition-colors group-hover:text-neon" /></div><h3 className="mt-auto pt-10 font-display text-base text-terminal-foreground transition-colors group-hover:text-neon">{project.title}</h3><p className="mt-3 text-sm leading-6 text-terminal-muted">{project.description}</p><div className="mt-5 flex flex-wrap gap-1.5">{project.tags.map((tag) => <span key={tag} className="font-display text-[10px] text-cyan">#{tag.replaceAll(" ", "")}</span>)}</div></a>)}</div></div></div>
      </section>
